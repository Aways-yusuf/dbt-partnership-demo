...
with source_data as (
    select * from {{ ref('int_city_enriched') }}
),
...
new_rows as (
    select
        {{ surrogate_key(['wwi_city_id']) }} as city_key,
        cast(wwi_city_id as int64) as wwi_city_id,
        city,
        state_province,
        country,
        continent,
        ...
        {{ hash_diff(['city', 'state_province', 'country', 'continent', 'sales_territory', 'region', 'subregion', 'location', 'latest_recorded_population']) }} as hashdiff
    from source_data
...
rows_to_close as (
    ...
    inner join new_rows nr
        on er.wwi_city_id = nr.wwi_city_id
    group by er.city_key, er.wwi_city_id
),
