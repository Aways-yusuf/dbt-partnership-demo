...
new_rows as (
    select
        {{ surrogate_key(['wwi_employee_id']) }} as employee_key,
        cast(wwi_employee_id as int64) as wwi_employee_id,
        employee,
        preferred_name,
        is_salesperson,
        valid_from,
        coalesce(valid_to, {{ end_of_time }}) as valid_to,
        {{ hash_diff(['employee', 'preferred_name', 'is_salesperson']) }} as hashdiff
    from source_data
...
rows_to_close as (
    ...
    inner join new_rows nr
        on er.wwi_employee_id = nr.wwi_employee_id
    group by er.employee_key, er.wwi_employee_id
),
...
closed_rows as (
    ...
    left join rows_to_close rtc
        on er.employee_key = rtc.employee_key
),
