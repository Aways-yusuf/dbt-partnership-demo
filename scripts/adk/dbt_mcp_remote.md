...
# 4. Configure secrets: create .env (see "Authentication" below).
#    Minimum: DBT_MCP_URL, DBT_PROD_ENVIRONMENT_ID, (DBT_API_TOKEN or DBT_API_TOKEN_FILE),
#    and for no API key: GOOGLE_GENAI_USE_VERTEXAI=TRUE, GOOGLE_CLOUD_PROJECT
```

### Authentication (avoid API key for the agent)

**Agent LLM (Gemini)** – no API key needed if you use Vertex AI:

- In `.env` set: `GOOGLE_GENAI_USE_VERTEXAI=TRUE`, `GOOGLE_CLOUD_PROJECT=your-project-id`
- Run once: `gcloud auth application-default login`
- The agent will use Vertex AI with Application Default Credentials instead of the Gemini API key.

**dbt MCP (dbt Cloud)** – token only:

- dbt Cloud’s remote MCP only supports **token** auth (Personal Access Token or Service Account Token). There is no OAuth or API-key alternative for this endpoint.
- You can either:
  - Set `DBT_API_TOKEN=your-token` in `.env`, or
  - Set `DBT_API_TOKEN_FILE=/path/to/file` where the file contains only the token (keeps the token out of `.env`).
- Get prod environment ID from dbt Cloud: Orchestration → environment ID.

### 429 RESOURCE_EXHAUSTED (Vertex AI rate limits)

If you see **429 Resource exhausted**:

1. **Restart** – Ensure the agent is using the defaults: `gemini-2.0-flash`, `max_output_tokens=1024`. Restart `adk web` or `adk run` after any `.env` change.

2. **Stay on Vertex, reduce load** – In `.env` set and restart:
   - `ADK_MCP_MAX_OUTPUT_TOKENS=1024` (default) or `512`
   - `GOOGLE_CLOUD_LOCATION=us-east1` or `europe-west1` (different region = different quota pool)

3. **Switch to Gemini API (often fixes 429)** – Use the Gemini API instead of Vertex; it has a separate quota pool. In `.env`:
   - `GOOGLE_GENAI_USE_VERTEXAI=FALSE` (or remove it)
   - `GOOGLE_API_KEY=your-api-key` (from [Google AI Studio](https://aistudio.google.com/apikey))
   Then restart. You no longer need `GOOGLE_CLOUD_PROJECT` for the LLM.

4. **Wait and retry** – 429 is often transient. Wait **1–2 minutes**, then try the same request again.

5. **Quota** – [GCP Console → Quotas](https://console.cloud.google.com/iam-admin/quotas), filter “Vertex AI”, request an increase for your project/region if 429 persists.

6. **Docs** – [ADK: Error code 429](https://google.github.io/adk-docs/agents/models/#error-code-429-resource_exhausted), [Vertex AI 429](https://cloud.google.com/vertex-ai/generative-ai/docs/error-code-429).
